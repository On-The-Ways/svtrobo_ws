...
└── YYYYMMDD_HHMMSS/                  # 录制会话（如 20260417_212712）
    ...
    ├── pointcloud/                    # 3D点云 (2x降采样)
    │   └── zed/                       # ZED XYZRGBA float32 (360x640, ~3.5MB/帧, ~1.4Hz)
...
> 采集时自动 2x 降采样（720x1280 → 360x640），每帧从 14MB 降至 3.5MB，每小时约 16GB（原始 64GB）。 |
...
| ZED 点云 | `pointcloud/zed/*.npz` | ~1.4 Hz | ZED SDK, 2x降采样 (360x640, ~3.5MB/帧) |
...
# data.shape = (360, 640, 4), dtype=float32 (2x降采样后)
