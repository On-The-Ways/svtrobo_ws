...
public:
	...
	RobStrite_Motor(uint8_t CAN_Id, rclcpp::Node* node, const std::string &command_tx);
	RobStrite_Motor(float (*Offset_MotoFunc)(float Motor_Tar) , uint8_t CAN_Id, rclcpp::Node* node, const std::string &command_tx);
